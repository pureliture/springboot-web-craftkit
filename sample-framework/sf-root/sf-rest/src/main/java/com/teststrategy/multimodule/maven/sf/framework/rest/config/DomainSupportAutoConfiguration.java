...
public class DomainSupportAutoConfiguration {
    ...
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(name = DomainApiProperties.COMPAT_CONFIG_PATH)
    public DomainApiProperties domainApiPropertiesCompat(Environment environment, ResourceLoader resourceLoader) {
        return new DomainApiProperties(environment, resourceLoader);
    }
}
