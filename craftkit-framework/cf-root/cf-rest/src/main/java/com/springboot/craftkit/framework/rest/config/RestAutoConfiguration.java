...
import com.springboot.craftkit.framework.rest.client.*;
import com.springboot.craftkit.framework.rest.client.*;
import com.springboot.craftkit.framework.rest.client.chain.UriTemplateHandlerInterceptorBinder;
import com.springboot.craftkit.framework.rest.client.chain.UriTemplateHandlerInterceptorChain;
import com.springboot.craftkit.framework.rest.client.chain.UriTemplateHandlerInterceptorFinalizer;
import com.springboot.craftkit.framework.rest.setting.*;
...
public class RestAutoConfiguration {
    ...
    @ConditionalOnMissingBean
    public DomainUriTemplateHandler domainUriTemplateHandler(
            DomainProperties domainProperties,
            ObjectProvider<DomainApiProperties> domainApiProperties,
            Environment environment) {
        return new DomainUriTemplateHandler(
                domainProperties,
    ...
    @Bean
    @ConditionalOnBean(DomainProperties.class)
    public UriTemplateHandlerInterceptorFinalizer uriTemplateHandlerInterceptorFinalizer() {
        return new UriTemplateHandlerInterceptorFinalizer();
    }

    @Bean
    @ConditionalOnBean(DomainProperties.class)
    @org.springframework.boot.autoconfigure.condition.ConditionalOnMissingBean
    public DtoUriTemplateHandler dtoUriTemplateHandler() {
        return new DtoUriTemplateHandler();
    }

    @Bean
    @ConditionalOnBean(DomainProperties.class)
    public UriTemplateHandlerInterceptorBinder uriTemplateHandlerInterceptorBinder(
            org.springframework.beans.factory.ObjectProvider<UriTemplateHandlerInterceptorChain> chainsProvider,
            UriTemplateHandlerInterceptorFinalizer finalizer) {
        return new UriTemplateHandlerInterceptorBinder(chainsProvider, finalizer);
    }

    @Bean
    @ConditionalOnBean(DomainProperties.class)
    public RestTemplateCustomizer uriTemplateHandlerChainCustomizer(
            UriTemplateHandlerInterceptorBinder binder) {
    ...
    @ConditionalOnProperty(prefix = HttpClientEvictorProperties.PREFIX, name = "enabled", havingValue = "true")
    public IdleConnectionEvictor idleConnectionEvictor(
            org.apache.hc.client5.http.impl.io.PoolingHttpClientConnectionManager connectionManager,
            HttpClientEvictorProperties evictorProperties) {
